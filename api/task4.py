...
def generate_indoor_event_plan():
    try:
        data = request.json
        space_and_budget = data.get('spaceAndBudget')

        # 장소 추천 프롬프트
        location_prompt = (
            f"다음 조건에 맞는 실내 이벤트 장소를 추천해 주세요. 지역 이름(예: 성수, 홍대, 가로수길 등)만 제시하세요:\n"
            f"1. 공간과 예산: {space_and_budget}\n"
            f"2. 주요 고객 관심사: {data.get('customerInterest')}\n"
            f"3. 이벤트 주제: {data.get('theme')}\n"
        )
        ...
        prompt = (
            ...
            f"1. 목적: {data.get('goal')}\n"
            f"2. 타겟층: {data.get('targetAudience')}\n"
            f"3. 공간과 예산: {space_and_budget}\n"
            f"4. 주요 고객 관심사: {data.get('customerInterest')}\n"
            f"5. 이벤트 주제: {data.get('theme')}\n"
